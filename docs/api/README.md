...
| PATCH | `/api/users/:userId/role` | 権限を変更 | **管理者のみ** | `204` | `400` `404` |
...
`PATCH /api/users/:userId/role` は**最後の管理者を降格できない**。権限を渡せるのは管理者だけなので、
ひとりもいない状態からは戻れない。判定と更新は1文で、2文に分けると最後の2人が同時に互いを
降格できる隙間ができる。最初に作られたアカウントは自動的に管理者になる（`src/worker/auth.ts`）。
