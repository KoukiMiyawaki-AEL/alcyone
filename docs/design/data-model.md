...
- **退会は「自分が作ったプロジェクト」だけでは終わらない。** `user.id` を指す列は
  6つあり、そのうち4つ（`todo_comments.authorId` / `todo_events.actorId` /
  `project_members.userId` / `project_members.addedBy`）は**他人のプロジェクトの中**にも現れる。
  1つでも残っていると外部キーが `user` 行の削除を拒み、退会そのものが失敗する。
  何を消して何を `null` にするかは行ごとに違う——一覧は
  [`docs/pii.md`](../pii.md)。
